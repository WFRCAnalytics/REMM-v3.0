from urbansim_wfrc.models import RegressionModel, SegmentedRegressionModel, \
    MNLLocationChoiceModel,SegmentedMNLLocationChoiceModel, \
    GrowthRateTransition, transition
from urbansim_wfrc.models.supplydemand import supply_and_demand
...
def to_frame(tbl, join_tbls, cfg, additional_columns=[]):
    ...
    # print(misc.column_list(tables, cfg.columns_used()))
...
def hedonic_estimate(cfg, tbl, join_tbls):
    ...
    # df.to_csv('buildings_rsh_davis.csv')
...
def full_transition(agents, agent_controls, year, settings, location_fname):
    ...
    if agents.name == 'jobs':
        pipelineJobs = pd.read_csv(os.path.join(misc.data_dir(), "pipelineJobs.csv"))
