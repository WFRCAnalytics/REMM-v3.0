...
def jobs_slc(jobs):
    df = jobs.to_frame(['county_id','building_type_id'])
    df.rename(columns={'county_id': 'cid'}, inplace=True)
    return df[df.cid==35]
...
def households(store, settings):
    ...
    p = store['parcels']
    p = p[['county_id']]
    p.rename(columns={'county_id': 'cid'}, inplace=True)
    b = store['buildings']
    b = b[['parcel_id']]
    df = df.merge(b, how ='left', left_on = 'building_id', right_index = True).merge(p, how = 'left', left_on = 'parcel_id' , right_index = True)
